...
public class OrderFillerBean extends AbstractJmsMessageDrivenBean {
	...
	private void sendPO(PurchaseOrder po) throws XMLException {
		...
		if (lodging != null)
			JMSUtils.sendMessage(JNDINames.WS_BROKER_MDB_QUEUE,
					JNDINames.DOC_TYPE, JNDINames.LODGING_ORDER, lodging
							.toXML(po.getPoId()));
		if ((depFlight != null) || (retFlight != null))
			JMSUtils.sendMessage(JNDINames.WS_BROKER_MDB_QUEUE,
					JNDINames.DOC_TYPE, JNDINames.AIRLINE_ORDER,
					getTransportationPO(depFlight, retFlight, po.getPoId()));
		if ((po.getActivities().length) != 0)
			JMSUtils.sendMessage(JNDINames.WS_BROKER_MDB_QUEUE,
					JNDINames.DOC_TYPE, JNDINames.ACTIVITY_ORDER,
					getActivityPO(activities, po.getPoId()));

	}

	private String getActivityPO(Activity[] acts, String poId)
			throws XMLException {
		...
		try {
			...
			for (int i = 0; i < acts.length; ++i) {
				...
				elem.appendChild(doc.createTextNode((new SimpleDateFormat(
						"MM-dd-yy")).format(acts[i].getStartDate().getTime())));
				actyElem.appendChild(elem);
				elem = doc.createElement("EndDate");
				elem.appendChild(doc.createTextNode((new SimpleDateFormat(
						"MM-dd-yy")).format(acts[i].getEndDate().getTime())));
				actyElem.appendChild(elem);
				elem = doc.createElement("HeadCount");
				elem.appendChild(doc.createTextNode(Integer.toString(acts[i]
						.getHeadCount())));
				actyElem.appendChild(elem);
			}
	...
	private String getTransportationPO(Transportation dep, Transportation ret,
			String poId) throws XMLException {
		...
		try {
			...
			if (dep == null) {
				elem.appendChild(doc.createTextNode((new SimpleDateFormat(
						"MM-dd-yy")).format(Calendar.getInstance().getTime())));
			} else {
				elem.appendChild(doc.createTextNode((new SimpleDateFormat(
						"MM-dd-yy")).format(dep.getDepartureDate().getTime())));
			}
			transportElem.appendChild(elem);
			...
			if (ret == null) {
				...
				elem.appendChild(doc.createTextNode((new SimpleDateFormat(
						"MM-dd-yy")).format(ret.getDepartureDate().getTime())));
			}
			transportElem.appendChild(elem);
			String headCount = dep != null ? Integer.toString(dep
					.getHeadCount()) : Integer.toString(ret.getHeadCount());
			elem = doc.createElement("HeadCount");
