...
public class PointbaseAccountDAO implements AccountDAO {
   ...
   public PointbaseAccountDAO() {
	   super();
	   final String dataSourceName = ServiceLocator.getInstance().getString(JNDINames.CUSTOMER_DATASOURCE);
    ...
    private void insertAccount(Account details) throws
        ...
        int resultCount = customerTemplate.update(INSERT_ACCOUNT_QUERY_STR, new CustomerPreparedStatementSetter(details));
        if ( resultCount != 1 ) {
    ...
    private boolean userExists (String userId) throws DAOSystemException {
    	try {
    		customerTemplate.queryForObject(SELECT_USER_ID_QUERY_STR, new Object[]{userId}, String.class);
    		return true;
    ...
    private Account selectAccount(String userId) throws DAOSystemException, AccountDAOFinderException {

		Object[] params = new Object[] {userId.trim()};
		try {
	    	Account account = (Account) customerTemplate.queryForObject(SELECT_ACCOUNT_QUERY_STR,params, new AccountRowMapper());
	    	return account;
		} catch (IncorrectResultSizeDataAccessException e) {
    ...
    private static class CustomerPreparedStatementSetter implements PreparedStatementSetter {
