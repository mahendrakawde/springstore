...
public class TransportSearchHTMLActionTest extends AbstractActionTests {
	...
	public void search() throws Exception {
		Cart cart = getAdventureComponentManager()
		...
		expect(catalogDAO.getTransportations(ORIGIN, DESTINATION, Locale.US))
				.andReturn(origins);
		expect(catalogDAO.getTransportations(DESTINATION, ORIGIN, Locale.US))
		...
		assertEquals(origins.size(), ((List) request
				.getAttribute("departure_result")).size());
		assertEquals(destinations.size(), ((List) request
				.getAttribute("return_result")).size());
	...
	public void searchBasedOnRequest() throws Exception {
		...
		expect(
				catalogDAO.getTransportations(REQUEST_ORIGIN, DESTINATION,
						Locale.US)).andReturn(origins);
		expect(
				catalogDAO.getTransportations(DESTINATION, REQUEST_ORIGIN,
						Locale.US)).andReturn(destinations);
		replay(catalogDAO);
