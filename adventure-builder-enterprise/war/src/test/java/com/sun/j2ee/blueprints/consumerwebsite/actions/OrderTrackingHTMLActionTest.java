...
public class OrderTrackingHTMLActionTest extends AbstractActionTests {
	...
	public void performNoOrderFoundException() throws Exception {
		expect(orderTrackingService.getOrderDetails(ORDER_ID)).andThrow(new OrderNotFoundException("Test Exception"));
	...
	public void performOrderNotFoundExceptionConversion() throws Exception {
		expect(orderTrackingService.getOrderDetails(ORDER_ID)).andThrow(new RuntimeException("Test Remote Exception"));
		replay(orderTrackingService);
		action.perform(request);
		verify(orderTrackingService);
	}
	...
	public void performOrderNotFoundExceptionNullPO() throws Exception {
		expect(orderTrackingService.getOrderDetails(ORDER_ID)).andReturn(new OrderDetails());
		replay(orderTrackingService);
		action.perform(request);
		verify(orderTrackingService);
	}
	...
	public void perform() throws Exception {
		...
		expect(orderTrackingService.getOrderDetails(ORDER_ID)).andReturn(orderDetails);
		replay(orderTrackingService);
		action.perform(request);
